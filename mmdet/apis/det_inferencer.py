...
class DetInferencer(BaseInferencer):
    ...
    def _load_weights_to_model(self, model: nn.Module,
        ...
        if checkpoint is not None:
            ...
            # print(checkpoint_meta['dataset_meta'])
            # import sys
            # sys.exit()
            # save the dataset_meta in the model for convenience
            if  'dataset_meta' in checkpoint_meta and isinstance(checkpoint_meta['dataset_meta'], dict):
                ...
                # import sys
                # sys.exit()
            elif  'dataset_meta' in checkpoint_meta and isinstance(checkpoint_meta['dataset_meta'], list): ###################
                # mmdet 3.x, all keys should be lowercase
                cl_all = []
                dataset_meta_all = dict()
                for i in range(len(checkpoint_meta['dataset_meta'])):
                    for cl in checkpoint_meta['dataset_meta'][i]['classes']:
                        cl_all.append(cl.lower())
                cl_set = tuple(set(cl_all))
                dataset_meta_all.update({'classes': cl_set})
                model.dataset_meta = dataset_meta_all
                print(dataset_meta_all)
    ...
    def _init_pipeline(self, cfg: ConfigType) -> Compose:
        ...
        # print(cfg.test_dataloader.pipeline)
    ...
    def __call__(
        ...
        for ori_imgs, data in (track(inputs, description='Inference')
            ...
            for pre in preds:
                pred_instances = pre.pred_instances.numpy()
                result = {
