#!/usr/bin/env python3
"""Teacher-style marker pipeline for OPT-RSVG with MTP fixed-class detector."""
...
_MTP_HD_ROOT = Path(__file__).resolve().parents[2] / "third_party" / "MTP" / "RS_Tasks_Finetune" / "Horizontal_Detection"
...
def patch_mtp_infer_cfg_for_square_resize(cfg_path: str) -> Config:
    cfg = Config.fromfile(cfg_path)
    cfg.model.backbone.pretrained = None
    cfg.model.backbone.use_checkpoint = False

    def _rewrite_pipeline(pipeline: List[Dict[str, Any]]) -> None:
        for step in pipeline:
            ...
            if step.get("type") == "LoadAnnotations":
                # Inference does not need GT annotations; removing this avoids
                # unnecessary assumptions about external annotation layout.
                step["_drop_for_infer"] = True
        pipeline[:] = [s for s in pipeline if not s.get("_drop_for_infer")]

    for attr in ("test_pipeline",):
        pipeline = cfg.get(attr)
    ...
    try:
        ...
        test_dataset = None
    if test_dataset is not None and hasattr(test_dataset, "pipeline"):
        _rewrite_pipeline(test_dataset.pipeline)
    ...
    try:
        ...
        val_dataset = None
    if val_dataset is not None and hasattr(val_dataset, "pipeline"):
        _rewrite_pipeline(val_dataset.pipeline)

    return cfg


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Teacher-style OPT-RSVG marker pipeline for MTP fixed-class detector.")
...
def parse_opt_target_classes(samples: List[Dict[str, Any]]) -> List[str]:
    ...
    for s in samples:
        ...
        if cls and cls not in names:
            names.append(cls)
    return [c for c in names if c in OPT_TO_DIOR_CLASS]
...
def suppress_overlapping_bboxes_by_iou_for_label(
    ...
    for cand in sorted_bboxes:
        ...
        for kept_box in kept:
            if calculate_iou(cand[:4], kept_box[:4]) > iou_threshold:
                should_keep = False
                break
        if should_keep:
            kept.append(cand)
...
def render_teacher_markers(
    ...
    *,
    ...
    for i, pred_bbox in enumerate(merged_pred_bboxes):
        ...
        draw_bbox(img, (xmin, ymin, xmax, ymax), color, line_thickness, str(marker_id), font_scale, text_thickness)
...
def main() -> None:
    ...
    cfg = patch_mtp_infer_cfg_for_square_resize(args.config)
    model = init_detector(cfg, args.checkpoint, device=args.device, palette="random")
    ...
    manifest = {
        "mode": "teacher_opt_rsvg_mtp_fixed_detector",
    ...
    print(f"[DONE] teacher-style OPT-RSVG markers (MTP fixed detector) -> {out_dir}")
